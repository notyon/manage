...
def register(app):
    ...
    async def check_force_sub(client, message):
        ...
        try:
            ...
            await send_log(f"🔒 User @{user.username or user_id} dimute karena belum join channel.", client)
    ...
    async def handle_unmute_button(client, callback_query):
        ...
        channel_id = get_force_channel(chat_id)
        if not channel_id:
            return await callback_query.answer("❌ Channel belum diset.", show_alert=True)
        ...
        try:
            ...
            await send_log(f"🔓 User @{callback_query.from_user.username or user_id} di-unmute (join channel).", client)
        except UserNotParticipant:
